...
*   **Sicherheitskriterien:** Prüft auf relative Pfade, das Fehlen von Passwörtern, Secrets, API-Keys (z. B. `AIzaSy`, das AI Feature Password), GCP Service Account JSONs (`private_key`), nicht getrackte `.env`-Dateien, URLs mit Token-Parametern und saubere Konfigurationsvorlagen (wie `.env.example` und `*.tfvars`).
...
### [stride-threat-model](../skills/stride-threat-model/SKILL.md)
*   **STRIDE-Sicherheitsbewertung:** Bietet Richtlinien für eine systematische Bedrohungsanalyse des Projekts basierend auf den sechs Säulen des STRIDE-Modells (Spoofing, Tampering, Repudiation, Information Disclosure, Denial of Service, Elevation of Privilege).
*   **Systemgrenzen und Datenflüsse:** Analysiert Einstiegspunkte (Tools, Workflows, Prompts) und Datenspeicherebenen zur Identifikation von Schwachstellen.
*   **Bedrohungsbericht:** Leitet die Erstellung eines strukturierten `threat_model.md` Berichts im Hauptverzeichnis des Projekts an.
